...
- A `development task` is a task that changes repository content and must be completed through the repository's defined development flow.
- A `non-development task` is a task that does not require repository changes through the repository's development flow, such as analysis, discussion, or information gathering.
...
## 8. Default Development and Review Flow
...
## 9. Default Completion Conditions
...
6. Move the task to `In Progress` and start development.
7. Enter the review step according to the current repository's rules.
8. Get a signed `Approve` from the Peer during the review step.
9. Complete merge or submission according to the current repository's rules.
10. Have the `Owner` explicitly move the task to `Done`.
